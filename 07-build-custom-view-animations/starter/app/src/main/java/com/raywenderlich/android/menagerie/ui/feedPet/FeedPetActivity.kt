...
class FeedPetActivity : AppCompatActivity() {
  ...
  private var vertikalPositionDifference = 0f
  ...
  private fun setupUi() {
    ...
    val springCookieX = SpringAnimation(
      binding.cookie,DynamicAnimation.TRANSLATION_X
    ).setSpring(springForce)

    val springChocolateCookieY = SpringAnimation(
      binding.chocolateCookie,DynamicAnimation.TRANSLATION_Y
    ).setSpring(springForce)

    val springCookieY = SpringAnimation(
      binding.cookie,DynamicAnimation.TRANSLATION_Y
    ).setSpring(springForce)

    binding.chocolateCookie.setOnTouchListener(buildTouchListener(
      springChocolateCookieX,
      springChocolateCookieY))

    binding.cookie.setOnTouchListener(buildTouchListener(
      springCookieX
      , springCookieY))
  }
  ...
  private fun buildTouchListener(
    ...
    when(event?.action) {
      MotionEvent.ACTION_DOWN -> {
        ...
        vertikalPositionDifference = event.rawY - (view?.y ?: 0f)
      ...
      MotionEvent.ACTION_MOVE -> {
        ...
        view.y = event.rawY - vertikalPositionDifference
      }
